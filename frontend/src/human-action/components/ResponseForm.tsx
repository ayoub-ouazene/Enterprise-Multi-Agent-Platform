...
export function ResponseForm({ action, onSubmit, isSubmitting }: {
  ...
  const [comment, setComment] = useState(() => sessionStorage.getItem(`orchestra.action-comment.${action.id}`) ?? '');
  ...
  return <div className="space-y-5">
    ...
    <label className="grid gap-2 text-sm font-medium">Comment{negative ? ' (required)' : ' (optional)'}<textarea rows={4} value={comment} onChange={event => { setComment(event.target.value); sessionStorage.setItem(`orchestra.action-comment.${action.id}`, event.target.value); }} className="rounded-lg border border-neutral-300 bg-white p-3 dark:border-neutral-700 dark:bg-neutral-950" /></label>
