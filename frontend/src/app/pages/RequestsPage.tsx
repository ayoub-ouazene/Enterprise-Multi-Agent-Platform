...
function RequestEmpty({ filtered, attentionOnly, clear }: { filtered: boolean; attentionOnly: boolean; clear: () => void }) {
  return <div className="rounded-card border border-dashed border-neutral-300 bg-white px-6 py-14 text-center dark:border-neutral-700 dark:bg-neutral-900"><Filter className="mx-auto text-neutral-300" size={34} /><h2 className="mt-4 font-semibold text-neutral-950 dark:text-white">{attentionOnly ? 'No requests require attention' : filtered ? 'No requests match these filters' : 'No requests yet'}</h2><p className="mx-auto mt-2 max-w-md text-sm text-neutral-500">{filtered ? 'Adjust or clear the current filters to see other authorized requests.' : 'Create a request when you need help or want to start a company process.'}</p><div className="mt-5 flex justify-center gap-2">{filtered && <Button variant="secondary" onClick={clear}>Clear filters</Button>}<Link to="/app/requests/new" className="inline-flex h-10 items-center rounded-md bg-primary-600 px-4 text-sm font-medium text-white">Create request</Link></div></div>;
}
